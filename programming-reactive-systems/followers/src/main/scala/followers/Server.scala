...
object Server extends ServerModuleInterface:
  ...
  val identityParserSink: Sink[ByteString, Future[Identity]] =
    reframedFlow.map(message => Identity.parse(message)).toMat(Sink.head)(Keep.right)
  ...
  val reintroduceOrdering: Flow[Event, Event, NotUsed] =
    Flow[Event].statefulMapConcat(() =>
        ...
        event =>
          def getBufferedEvents(seqNr: Int): List[Event] =
            if (buffer.contains(seqNr))
              val event = buffer(seqNr)
              buffer -= seqNr
              event :: getBufferedEvents(seqNr + 1)
            else
              Nil
          if (event.sequenceNr == lastSeqNr)
            val eventsBuffered = getBufferedEvents(lastSeqNr + 1).reverse
            lastSeqNr += 1 + eventsBuffered.length
            event :: eventsBuffered
          else
  ...
  val followersFlow: Flow[Event, (Event, Followers), NotUsed] =
    Flow[Event].statefulMapConcat(() =>
        var followers: Followers = Map()
        def addFollower(from: Int, to: Int): Unit =
          followers += (from, followers.getOrElse(from, Set()) + to)
        def removeFollower(from: Int, to: Int): Unit =
          followers += (from, followers.getOrElse(from, Set()) - to)
        event =>
          event match
  ...
  def isNotified(userId: Int)(eventAndFollowers: (Event, Followers)): Boolean =
    eventAndFollowers match
      case (Event.Follow(_, _, to), _) if userId == to =>
        true
      case (Event.Broadcast(_), _) =>
        true
      case (Event.PrivateMsg(_, _, to), _) if userId == to =>
        true
      case (Event.StatusUpdate(_, from), followers) if followers.get(userId).exists(_.contains(from)) =>
        true
      case _ =>
        false
...
class Server(using ExecutionContext, Materializer)
  ...
  def outgoingFlow(userId: Int): Source[ByteString, NotUsed] =
    broadcastOut
      .filter(eventAndFollowers => isNotified(userId)(eventAndFollowers))
      .map(eventAndFollowers => eventAndFollowers._1.render)
  ...
  def clientFlow(): Flow[ByteString, ByteString, NotUsed] =
    ...
    val incoming: Sink[ByteString, NotUsed] = identityParserSink
      .mapMaterializedValue(future =>
        future.map(value => clientIdPromise.complete(Try(value)))
        NotUsed
      )
