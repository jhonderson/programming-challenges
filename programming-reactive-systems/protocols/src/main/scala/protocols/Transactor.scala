...
object Transactor:
  ...
  private def inSession[T](rollbackValue: T, sessionTimeout: FiniteDuration, sessionRef: ActorRef[Session[T]]): Behavior[PrivateCommand[T]] =
    Behaviors.receive {
      case (ctx, Committed(replyTo, value)) =>
        idle(value, sessionTimeout)
      case (ctx, RolledBack(replyTo)) =>
        ctx.stop(replyTo)
  ...
  private def sessionHandler[T](currentValue: T, commit: ActorRef[Committed[T]], done: Set[Long]): Behavior[Session[T]] =
    Behaviors.receive {
      case (ctx, Extract(projector, replyTo)) =>
        val projectedValue = projector(currentValue)
        replyTo ! projectedValue
        Behaviors.same
      case (ctx, Modify(transformation, id, reply, replyTo)) if (done.contains(id)) =>
        replyTo ! reply
        Behaviors.same
      case (ctx, Modify(transformation, id, reply, replyTo)) =>
        val changedValue = transformation(currentValue)
        replyTo ! reply
        sessionHandler(changedValue, commit, done + id)
